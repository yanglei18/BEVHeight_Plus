...
model = dict(
    ...
    img_view_transformer=dict(
        ...
        depthnet_cfg=dict(use_dcn=False, use_height=use_height),
        downsample=16),
    img_bev_encoder_backbone=dict(
        type='CustomResNet',
        numC_input=numC_Trans_Bev * (len(range(*multi_adj_frame_id_cfg))+1),
        num_channels=[numC_Trans_Bev * 2, numC_Trans_Bev * 4, numC_Trans_Bev * 8]),
    img_bev_encoder_neck=dict(
        type='FPN_LSS',
        in_channels=numC_Trans_Bev * 8 + numC_Trans_Bev * 2,
        out_channels=256),
    pre_process=dict(
        type='CustomResNet',
        numC_input=numC_Trans_Bev,
        num_layer=[2,],
        num_channels=[numC_Trans_Bev,],
        stride=[1,],
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
        data_root=data_root,
        ann_file=data_root + 'bevheight_plus_nuscenes_infos_all.pkl',
