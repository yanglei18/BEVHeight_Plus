...
def load_calib_kitti(calib_file):
    with open(calib_file, 'r') as csv_file:
        ...
        sensor_params = {
            "CAM_FRONT": (P2, Tr_velo2cam[:3, :3], Tr_velo2cam[:3, 3]), 
        }
...
def main():
    ...
    parser.add_argument("--data_root", type=str,
                        default="data/v2x-radar-v-kitti",
                        help="Path to Dataset root in KITTI format")
    args = parser.parse_args()

    kitti_root = args.data_root
    prefix = "v2x-radar-v"
    train_infos = generate_info_kitti(kitti_root, split='train_hom')
    val_infos = generate_info_kitti(kitti_root, split='val_hom')
