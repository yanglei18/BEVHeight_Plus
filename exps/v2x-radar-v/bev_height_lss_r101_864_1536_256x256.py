...
return_depth = False
data_root = "data/v2x-radar-v-kitti/"
gt_label_path = "data/v2x-radar-v-kitti/training/label_2"
...
class BEVHeightPlusLightningModel(LightningModule):
    ...
    def __init__(self,
        ...
        self.model = BEVHeightPlus(self.backbone_conf, self.head_conf, is_train_height=self.return_depth)
        self.mode = 'valid'
    ...
    def train_dataloader(self):
        train_dataset = NuscMVDetDataset(
            ...
            info_path=os.path.join(data_root, 'v2x-radar-i_12hz_infos_train_hom.pkl'),
            is_train=True,
    ...
    def val_dataloader(self):
        val_dataset = NuscMVDetDataset(
            ...
            classes=self.class_names,
            data_root=self.data_root,
            info_path=os.path.join(data_root, 'v2x-radar-i_12hz_infos_val_hom.pkl'),
            is_train=False,
            img_conf=self.img_conf,
            num_sweeps=self.num_sweeps,
            sweep_idxes=self.sweep_idxes,
...
def main(args: Namespace) -> None:
    ...
    checkpoint_callback = ModelCheckpoint(dirpath='./outputs/bev_height_lss_r101_864_1536_256x256/checkpoints', filename='{epoch}', every_n_epochs=2, save_last=True, save_top_k=-1)
    ...
    if args.evaluate:
        ...
        if os.path.exists("pretrain_ckpt/pretrain.ckpt"):
            model = BEVHeightPlusLightningModel.load_from_checkpoint("pretrain_ckpt/pretrain.ckpt")
        trainer.fit(model)
